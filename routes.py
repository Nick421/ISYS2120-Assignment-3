...
def rank():
    """
    Shows a list of all deparment by ranking.
    """
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))
    departments = database.productivity_rank()
...
def spending():
    """
    Shows a list of spending per month.
    """
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))
    spendings = database.most_spent_per_month()
...
@app.route('/department_manager_phone')
def department_manager_phone():
    """
    Shows a list of phone numbers.
    """
    # Check if the user is logged in, if not: back to login.
    if('logged_in' not in session or not session['logged_in']):
        return redirect(url_for('login'))
    phones = database.manager_department_phone()
    if phones is None:
        page['bar'] = False
        flash('Error communicating with database')
        phones = []
    return(render_template('department_manager_phone.html',
                           page=page,
                           session=session,
                           phones=phones))
